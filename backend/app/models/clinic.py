...
class ClinicMembership(Base):
    ...
    role: Mapped[str] = mapped_column(String(20), nullable=False)  # owner | admin | doctor | receptionist
    ...
    __table_args__ = (
        # Partial unique: soft-deleted memberships must not block re-invites
        Index(
            "uq_membership_user_clinic",
            "user_id",
            "clinic_id",
            unique=True,
